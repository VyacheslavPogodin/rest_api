...
# Запрос списка объектов архивов    
    ...
    # OBIS код - 1.0.98.2.0.255 03 Журнал ежесуточных показаний. 
        ...
        general_function.print_term_and_write_log('\t\t'+ str(send[47]*256+send[48])+'-'+str(send[49]).zfill(2)+'-'+str(send[50]).zfill(2)+' '+str(send[52]).zfill(2)+':'+str(send[53]).zfill(2)) 
        general_function.print_term_and_write_log(send, )
...
# Запрос списка объектов журналов
    ...
    # OBIS код - 0.0.99.98.9.255 03 Журнал Параметры качества сети. 
        ...
        general_function.print_term_and_write_log(f'Получение <Журнала параметры качества сети> OBIS 1.0.99.98.9.255, адрес {send[3]}')
        general_function.print_term_and_write_log('\t\t'+ str(send[47]*256+send[48])+'-'+str(send[49]).zfill(2)+'-'+str(send[50]).zfill(2)+' '+str(send[52]).zfill(2)+':'+str(send[53]).zfill(2)) 
        general_function.print_term_and_write_log(send, )
...
# Запрос значений журналов    
    ...
    elif send[14:23] == b'\x00\x07\x01\x00^\x07\x00\xff\x02':
        general_function.print_term_and_write_log(f'Получение <Параметров качества> OBIS 1.0.99.98.13.255, адрес {send[3]}')
        general_function.print_term_and_write_log(send, )
