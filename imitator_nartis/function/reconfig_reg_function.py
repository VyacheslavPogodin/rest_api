...
def get_reconfig_register(send, ser, dict_device,):
    ...
    answer_data = binascii.unhexlify(b'e6e700c401c10001010205090c' + binascii.hexlify(date_send) +b'16'+reconfig_constant['event']+b'0901'+reconfig_constant['cannel']+b'0901'+reconfig_constant['client']+b'06'+reconfig_constant['uptime'])
    heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0'+general_function.len_send(answer_data)+send[4:5]+send[2:4]+b'R')+answer_data)+b'~'
    ser.write(heading_answer)
    general_function.print_term_and_write_log(heading_answer)