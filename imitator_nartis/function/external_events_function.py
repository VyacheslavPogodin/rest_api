...
def get_external_events(send, ser, dict_device,):
    ...
    answer_data = binascii.unhexlify(b'e6e700c401c10001020203090c' + binascii.hexlify(date_send_1) +b'16'+external_constant['event']+b'06'+external_constant['uptime']+\
        b'020e090c'+ binascii.hexlify(date_send_2)+b'16'+external_constant['event']+b'06'+external_constant['uptime'])
    heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0'+general_function.len_send(answer_data)+general_function.form_answer_adres(send)+b'R')+answer_data)+b'~'
    ser.write(heading_answer)
    general_function.print_term_and_write_log(heading_answer)
