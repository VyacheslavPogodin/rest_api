...
def assotiate_dlms(send):
    ...
    answer_data = b'\x81\x80\x14\x05\x02\x00\x80\x06\x02\x00\x80\x07\x04\x00\x00\x00\x01\x08\x04\x00\x00\x00\x01'
    heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0'+general_function.len_send(answer_data)+send[4:5]+send[2:4]+b's')+answer_data)+b'~'  
...
def auth_dlms_password(send):
    ...
    answer_data = b'\xe6\xe7\x00a)\xa1\t\x06\x07`\x85t\x05\x08\x01\x01\xa2\x03\x02\x01\x00\xa3\x05\xa1\x03\x02\x01\x00\xbe\x10\x04\x0e\x08\x00\x06_\x1f\x04\x00\x00\x10\x1d\x00d\x00\x07'        
    heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0'+general_function.len_send(answer_data)+send[4:5]+send[2:4]+b'0')+answer_data)+b'~'
    return heading_answer
...
def auth_dlms_cripto(send, ser, dict_device,):
    ...
    while n<1:    
        ...
        if send[5:6] == b'2':
            ...
            heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0'+general_function.len_send(answer_data)+send[4:5]+send[2:4]+b'2')+answer_data)+b'~'
            ser.write(heading_answer)
            general_function.print_term_and_write_log(heading_answer)
            send = b''
        else:
...
def get_date(send):
    ...
    heading_answer = b'~'+general_function.crc16(general_function.crc16(b'\xa0\x1f'+send[4:5]+send[2:4]+b'R')+b'\xe6\xe7\x00\xc4\x01\xc1\x00\t\x0c'+date)+b'~'
    return heading_answer
