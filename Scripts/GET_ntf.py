...
url_2 = 'http://10.10.29.110/api/v1/ntpservers/73'


payload = {"host":"ntp_test","port":129}
#payload_2 =  {'mode':'conf_out'}
#response = requests.get(url_2, headers=head, timeout=10)
#status_conf = response.json()['data']['status_id']
#cheng_status_conf = requests.post(url_2, headers=head, data=payload)
response = requests.get(url_2, headers=head, timeout=10)
#dict_ntp = response.json()[len(response.json())-1]
#print(type(response.json()[len(response.json())-1]))

dict_1 = {'name': 'NotFound',
'message': 'Page not found',
'code': 404,
'className': 'not-found',
'data': {'url': '/api/v1/ntpservers/73'}, 'errors': {}}

print(response.json())
print(dict_1)
print(response.json() == dict_1)
